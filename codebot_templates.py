...
# Immer davor: In line x you violated a convention/you made an error.
...
templates = {
        ...
        'C0202': make_templates('A class method should always have cls as first name.', 'Always use cls for the first argument to class methods. The cls name is used to easily differentiate class methods from instance methods. Always use self for the first argument to instance methods.', [2]),
        ...
        'E0107': make_templates('You have used an operator which does not exist in Python. Remember that for example the operators -- and ++ from C cannot be used in Python. Use += or -= instead!', 'You used a non-existent operator in this line, possibly a wrong pre- or post-increment or decrement operator.', [4]),
        'E1102': make_templates('You accessed a non-callable object.','You accessed a non-callable object', [0]),
        'E0211': make_templates('The first argument of a method in Python must be the object on which the method is invoked. Use ''self'' as the first argument to instance methods and ''cls'' as first argument to class methods.', 'You forgot to define an argument for a method. Remember that the bound instance is always the first argument.', []),
                 }
